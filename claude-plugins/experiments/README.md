...
Minor sibling of `/experiments:commander-update-deep-patch` — identical deep cross-project flow (research deduplicated by package, universal-only findings, four-option gate `apply-all` / `apply-bumps-only` / `pick-subset` / `cancel`, per-project changeset gate for improvements after the bumps loop), only the level/target are `minor`. Thin wrapper over `commander-update-orchestrator` (`level: "minor"`, `target: "minor"`, `mode: "deep"`). The synthesized `dossier.md` ends with the script-assembled `## Changelogs` chronology section. Plan dirs live under `~/.claude/experiments/plans/commander-deep-minor-<unix-ts>/` with the 10-day stale-cleanup. Inherits every hard rule from the shallow command plus `npm-update-deep-minor` — no tests, no lint, no build, no commits, never mutates the registry, never auto-executes an override.
...
Major sibling of `/experiments:commander-update-deep-minor` — identical deep cross-project flow (research deduplicated by package, four-option gate), with research weighted toward **breaking changes & migration** and a `## PR plan` (from `partition-breaking-changes`, retained legacy section name) appended by the orchestrator to the surfaced dossier digest. Thin wrapper over `commander-update-orchestrator` (`level: "major"`, `target: "major"`, `mode: "deep"`). The on-disk `dossier.md` carries `## Breaking changes & migration` + `## Changelogs`; improvements **and** reviewed migration edits land through the per-project changeset gate. v1 isolation caps at **one worktree per project** (per-(project,bucket) deferred). Inherits every hard rule from the shallow command plus `npm-update-deep-major` — never commits/pushes/opens PRs autonomously (worktree isolation allowed, opt-in default `none`), never mutates the registry, never auto-executes an override.
...
Engines sibling of `/experiments:commander-update-deep-major` — identical deep cross-project flow (four-option gate, per-project changeset gate for migration + improvement edits), with research over **engine release notes** (Node/pnpm/npm/yarn/Deno/Bun) **deduplicated once per engine/version** and weighted toward breaking changes/migration. Thin wrapper over `commander-update-orchestrator` (`level: "engines"`, `target: "engines"`, `mode: "deep"`). The synthesized `dossier.md` carries `## Breaking changes & migration` + `## Changelogs` (engine notes) and **no `## PR plan`** — an engine bump is a single coordinated co-upgrade (Node + its PM), so `partition-breaking-changes` does not apply. v1 isolation caps at one worktree per project. Inherits every hard rule: never commits/pushes/opens PRs autonomously (worktree isolation allowed, opt-in default `none`), never mutates the registry, runtime surfaces only (publishable support ranges untouched).
...
Same scope as `npm-update-patch` (patch-level, semver-safe, manifest bump + one install) but with **research**: every changelog is fetched in parallel by subagents who cross-reference it against this codebase, then a synthesizer teammate (compliance-checked) writes a single integrated `dossier.md` of (a) applicable improvements and (b) workarounds-resolved-by-upgrade, plus the bump set. The user picks `apply-all` / `apply-bumps-only` / `pick-subset` / `cancel`; improvements land through the changeset gate. Plan dir lives under `~/.claude/experiments/plans/` with stale-cleanup (>10 days) prompted on each invocation. Never commits/pushes/opens PRs autonomously.
...
Minor sibling of `/experiments:npm-update-deep-patch` — same deep single-project flow (parallel changelog research, dossier synthesis by the workflow's synthesizer teammate — compliance-checked, `apply-all` / `apply-bumps-only` / `pick-subset` / `cancel` gate), only the level differs. Scans at `level=minor`, runs `parallel-research-workflow` at `level: "minor"`, and applies bumps via the shared `apply-npm-updates` skill (generic-only — the deep path consults no override registry). The synthesized `dossier.md` carries a `## Minor bump set` table and a final `## Changelogs` chronology section. Never commits/pushes/opens PRs autonomously.
...
Major sibling of `/experiments:npm-update-deep-minor` — same deep single-project flow (parallel changelog research, dossier synthesis by the workflow's synthesizer teammate — compliance-checked, four-option gate), with research weighted toward **breaking changes & migration**. After research the accepted set is partitioned into PR-sized buckets (`partition-breaking-changes`) surfaced as a `## PR plan` in the dossier digest (retained legacy section name); optionally each bucket applies into its own worktree (`update-isolation`). The synthesized `dossier.md` carries `## Breaking changes & migration` + `## Major bump set` + `## Changelogs`. On `apply-all` improvements **and** reviewed migration edits land through the changeset gate. Deps are written exact (no `^`/`~`); the deep path consults no override registry. Never commits/pushes/opens PRs autonomously (worktree isolation allowed, opt-in default `none`).
...
Deep sibling of `/experiments:npm-update-engines` — same engines-level toolchain bump (detect → resolve → pin + align runtime surfaces), with research over **engine release notes** (Node/pnpm/npm/yarn/Deno/Bun) weighted toward breaking changes/migration. Drives `detect-toolchain-surfaces` → `parallel-research-workflow(level=engines)` → `apply-engine-bumps` + a changeset-gated migration round. The synthesized `dossier.md` carries `## Breaking changes & migration` + `## Engines bump set` + `## Changelogs` (engine notes) and **no `## PR plan`**. On `apply-all` reviewed migration + improvement edits land through the changeset gate; rejection preserves the bumps. Never commits/pushes/opens PRs autonomously (worktree isolation allowed, opt-in default `none`).
...
Session-start stack detection and skills.sh install. Reads the root `package.json` (plus `components.json` and `nx.json`) for react, next, shadcn, vue, svelte, nx, expo, vite, solid-js and angular signals, maps the detected tags to a curated manifest, installs whatever is missing via `bunx skills add` — batched per source repo — and ensures postinstall persistence. Companion to `skills-update-check`, which keeps those same skills current.

### `hookify`

Converts deterministic CLAUDE.md/AGENTS.md instructions into enforced Claude Code hooks. Reads the project's instruction files, skips managed sections, and identifies the single highest-impact hookifiable rule — an always/never paired with a concrete CLI action — then proposes it and implements the hook end-to-end on confirmation. One rule per invocation. Requires `jq` and stops if it is missing.
...
Multi-phase parallel-subagent orchestration over a pre-grouped package set: phase 0 stale-plan cleanup (>10d), phase 1 fetches every changelog via `experiments:npm-changelog`, phase 2 cross-references against this codebase (single-project mode) or produces universal findings only (cross-project mode), phase 3 verifies integrity (`retry-failed` / `continue-without` / `abort`), phase 4 dispatches a synthesizer teammate that writes `dossier.md` (two-layer compliance check). Persists artifacts under `~/.claude/experiments/plans/<slug>-<level>-<unix-ts>/`. The `mode` input (`single-project` default, `cross-project` for commander deep) selects the subagent prompt template, plan-dir layout (`scan.json` vs `scan-by-project.json` + `cross-project-plan.json`), and `dossier.md` template (single-project bump-set vs cross-project bump-set with `affects projects:` tags). Reusable across `npm-update-deep-{patch,minor,major,engines}` and `commander-update-deep-{patch,minor,major,engines}` — only the level and mode change.

### `npm-update-deep-orchestrator`

Single-project analog of `commander-update-orchestrator`, and the single source of truth for the deep pipeline: scan → `group-packages-for-research` → `parallel-research-workflow` → teammate-synthesized `dossier.md` → user-gated bumps apply → per-project changeset gate for improvements and migration edits. The four `/experiments:npm-update-deep-*` commands are thin entry points that invoke it once with their fixed `level`; every level difference lives in this file's delta table alone, so a behavioral rule cannot drift between levels. The main window holds only paths and bounded digests — never changelog bodies or the dossier itself. Never runs tests/lint/build; never commits/pushes/opens PRs autonomously.
...
Prevention half of the comment-discipline pair, and owner of the canonical comment policy at `skills/writing-comments/reference/policy.md`. Applies from invocation onward, including to files already touched earlier in the session; it performs no retroactive sweep. Once per session it checks `~/.claude/CLAUDE.md` for a one-line pointer to itself and _proposes_ adding one when absent — asked once and never re-offered after a decline; it never writes user configuration without explicit confirmation.
