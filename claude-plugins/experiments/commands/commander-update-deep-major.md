...
> **Major updates may include breaking changes.** The on-disk `dossier.md` carries a `## Breaking changes & migration` section and the script-assembled `## Changelogs` chronology; the surfaced digest references the dossier by path and appends the `## PR plan` (ordered buckets + count-by-policy summary) inline. Review before applying. This command never commits or PRs.
