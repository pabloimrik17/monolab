...
The workflow ships two research contracts selected by the `mode` input. Phases 0, 1 (batching + hard-wall fallback), 3 (integrity gate + retry-failed), end-of-flow cleanup, per-group `_meta.json` schema, and field naming conventions are identical across both modes. Mode affects three things only: the subagent prompt template (phase 1+2 wording), the on-disk plan-dir layout (single-project vs cross-project artifacts), and the phase 4 `plan.md` template.
...
The cross-project mode SHALL NOT introduce new phases or change phase transition order. The only mode-conditional differences are the three listed in the rows above the bottom row of the table (subagent prompt, plan-dir layout, plan.md template) plus the `_meta.json.mode` field.
