# commander-add-command Specification

## Purpose

Defines the `/commander:add` slash command — the user-facing entry point for registering a project in the user-scoped Commander registry at `<HOME>/.claude/commander/projects.json`. The command captures project metadata (name, path, keywords, description, special rules, repoType) using an A→B→C priority chain (explicit args → Haiku auto-detection → AskUserQuestion), invokes the `commander-normalize` skill to produce final controlled-vocabulary keywords, handles single-repo / single-project-monorepo / multi-project-monorepo topologies, and persists via the `commander-registry` `add` operation. The command is owned by the `commander` plugin.
