# npm-update-deep-engines-command Specification

## Purpose

The `/experiments:npm-update-deep-engines` command is the deep single-project sibling of `/experiments:npm-update-deep-major`, operating exclusively at **engines level** with deep research over engine release notes. It detects toolchain surfaces, researches Node/pnpm/npm/yarn/Deno/Bun release notes via `parallel-research-workflow`, then drives a user-gated `apply-engine-bumps` followed by migration edits via the changeset gate. It never invokes npm dependency scan/apply or `ncu`, and PR partitioning does not apply to a single coordinated engine bump.
