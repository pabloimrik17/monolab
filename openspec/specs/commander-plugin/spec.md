# commander-plugin Specification

## Purpose

Defines the `commander` Claude Code plugin: the CRUD interface over the user-scoped Commander project registry at `<HOME>/.claude/commander/projects.json`. The plugin houses the four CRUD slash commands (`/commander:add`, `/commander:list`, `/commander:update`, `/commander:delete`) and the `commander-normalize` skill. It follows standard Claude Code plugin structure, is a pnpm workspace member, and ships independently of `experiments` via release-please using the `commander--v{version}` tag prefix.
