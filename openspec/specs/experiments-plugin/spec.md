...
The `experiments` plugin SHALL include `commands/commander-delete.md`.

The command file SHALL have YAML frontmatter with a non-empty `description` field. The command SHALL be invocable as `/experiments:commander-delete`.

#### Scenario: Command file exists

- **WHEN** examining `claude-plugins/experiments/commands/`
- **THEN** `commander-delete.md` SHALL exist

#### Scenario: Frontmatter present

- **WHEN** reading `commands/commander-delete.md`
- **THEN** the file SHALL have YAML frontmatter with a non-empty `description` field

#### Scenario: Command invocable

- **WHEN** the user types `/experiments:commander-delete`
- **THEN** Claude SHALL execute the command instructions
...
- **WHEN** the user invokes `/experiments:commander-delete investlab` (or `--name investlab`)
...
- **WHEN** the user invokes `/experiments:commander-delete` with no arguments
...
- **WHEN** the user invokes `/experiments:commander-delete`
...
- **WHEN** the user invokes `/experiments:commander-delete <name>` with a `name` not present in the registry
