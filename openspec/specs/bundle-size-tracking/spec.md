...
**Given** the monorepo contains published packages (react-hooks, react-clean, ts-configs, ts-types)
...
**When** the bundle size upload script runs
**Then** the script skips bundle analysis for ts-configs
**And** the workflow logs indicate "Skipping bundle analysis for ts-configs (not affected by PR)"
**And** Codecov retains the baseline bundle size for ts-configs (does not reset to 0)

#### Scenario: Protected branches establish complete bundle baselines

**Given** code is pushed to main, develop, or pre branch
**When** the build completes
**Then** the bundle size script uploads all 4 packages unconditionally
...
**And** skips upload for react-hooks, ts-configs, and ts-types
**And** Codecov maintains baseline bundle sizes for the 3 non-uploaded packages
...
**Given** ts-types baseline is 670 bytes on main
**And** a pull request affects only react-hooks (not ts-types)
**When** bundle stats are uploaded (only react-hooks)
**Then** Codecov retains ts-types's 670 byte baseline
**And** subsequent PRs affecting ts-types compare against 670 bytes
...
**Given** ts-types and ts-configs are not affected
**When** all Codecov upload scripts run
...
**Then** the script loops through all 4 packages: react-hooks, react-clean, ts-configs, ts-types
