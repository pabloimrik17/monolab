# commander-update-deep-engines-command Specification

## Purpose

The `/experiments:commander-update-deep-engines` command is the deep cross-project sibling of `/experiments:commander-update-deep-major` and the cross-project counterpart of `/experiments:npm-update-deep-engines`. It is a thin wrapper that invokes the `commander-update-orchestrator` skill exactly once with the deep engines input set and surfaces the orchestrator's output verbatim, including the engine release-note `## Breaking changes & migration` and `## Changelogs` sections. It accepts no arguments or flags, inherits every orchestrator and deep-flow hard rule, and never modifies publishable-library `engines.node` support ranges.
