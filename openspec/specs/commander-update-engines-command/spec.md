# commander-update-engines-command Specification

## Purpose

The `/experiments:commander-update-engines` command is the shallow cross-project sibling of `/experiments:commander-update-major`, operating at **engines** level. It is a thin wrapper that invokes the `commander-update-orchestrator` skill exactly once with the shallow engines input set and surfaces the orchestrator's output verbatim. It accepts no arguments or flags, inherits every orchestrator hard rule, and never modifies publishable-library `engines.node` support ranges.
