...
The `/experiments:commander-update-deep-patch` slash command drives a cross-project, deep-research patch update flow. It invokes the `commander-update-orchestrator` skill exactly once with `level: "patch"`, `target: "patch"`, and `mode: "deep"`. Every action — scan, research, plan rendering, confirmation gate, sequential per-project apply, post-bumps plan-mode improvement round, and summary — happens inside the orchestrator. The command itself owns only frontmatter, the single Skill invocation, and verbatim surfacing of the orchestrator's output.
