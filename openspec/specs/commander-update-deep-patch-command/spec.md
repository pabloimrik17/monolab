# commander-update-deep-patch-command Specification

## Purpose

The `/experiments:commander-update-deep-patch` slash command drives a cross-project, deep-research patch update flow. It invokes the `commander-update-orchestrator` skill exactly once with `level: "patch"`, `target: "patch"`, and `mode: "deep"`. Every action — scan, research, dossier gate rendering, confirmation gate, sequential per-project apply, post-bumps per-project changeset gate, and summary — happens inside the orchestrator. The command itself owns only frontmatter, the single Skill invocation, and verbatim surfacing of the orchestrator's output.
