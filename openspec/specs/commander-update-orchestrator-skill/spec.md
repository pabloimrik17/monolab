...
**Step 6.5.3b — Recall prior run knowledge.** Between 6.5.3 and 6.5.4 the skill SHALL invoke `recall-run-knowledge` with the `groups[]` emitted by 6.5.2, the `level`, and `mode: cross-project`, and SHALL pass its result unchanged into 6.5.4 as the workflow's optional `priorKnowledge` input only when `baseAbsent` is false, `error` is absent, and a hit or related entry exists. The step SHALL sit after grouping — it consumes the emitted groups' `bucketKey` — and before the workflow invocation, because `priorKnowledge` reaches the research subagents through the workflow and not around it. A hit SHALL NOT remove a package from its group; the package still fetches its changelog and still appears in the cross-project bump set. The skill SHALL NOT open a run note or a package hub: it SHALL surface exactly one recall digest — the count line on success, `Knowledge: no base at <root>` when `baseAbsent` is true, or `Knowledge: recall failed (<reason>)` when `error` is present or no usable matcher result exists. When the knowledge base is absent, recall SHALL preserve the complete `{ root, baseAbsent: true, hits: [], related: [], summary }` result and 6.5.4 SHALL be invoked with no `priorKnowledge` input, leaving the rest of the deep run byte-for-byte as it was before this step existed. A failure result remains complete at the caller and is also omitted from `priorKnowledge`.
...
When `mode === "deep"` and the gate option is `apply-all`, `apply-bumps-only`, or `pick-subset`, Step 10 SHALL be split:
...
- **Step 10b — Per-project changeset gate round** (fires only when the gate option was `apply-all` or selected improvements through `pick-subset`, at least one project either applied bumps successfully or has a non-empty improvement-only subset, and the dossier contains at least one improvement bullet): for each eligible project, sequentially, the skill SHALL run the per-project apply gate defined by the experiments-plugin requirements "Per-project apply gate with turn-boundary pause" and "Human approval gate interface". Failed and pending projects are excluded; a project with no selected bumps and a non-empty improvement subset is eligible:
...
- **Step 10b.5 — Run knowledge persistence** (fires whenever at least one project reached apply through Step 10a or an improvement-only Step 10b round: after the last project's Step 10b round, or directly after Step 10a when no changeset round applied, and always before Step 10c end-of-flow cleanup): the skill SHALL

  1. Write `_meta.json.phase` as `"done"` atomically (temp sibling, then rename); `"executing"` SHALL have been written the same way before the first bump mechanism invocation or improvement-only Step 10b round.
  2. Assemble the outcome object (the store's `outcome.json` shape, without `recordedAt`) carrying `runId`, `level`, `mode`, the selected `gateOption`, and **one `projects[]` entry per project** that reached apply — either through its bump mechanism or an improvement-only Step 10b gate. Each entry carries `projectName`, mechanism `apply-npm-updates` (dependency levels) or `apply-engine-bumps` (`level=engines`), `bumps` set to the returned result fragment verbatim when the mechanism ran, and `changeset` recording that project's gate `status` (`approved`, `verification-failed`, `rejected`, `skipped`, `not-run`, `unknown`), its run-dir-relative `path` (or `null`), and its applicable and inapplicable counts (`null` when the project has no changeset). When no bump mechanism ran for an improvement-only project, `bumps` SHALL be the clean no-bump fragment `{ "appliedGeneric": [], "appliedOverrides": [], "installRan": false, "logPath": null, "failure": null }` for dependency levels or `{ "resolvedTargets": {}, "applied": [], "skipped": [], "droppedHashes": [] }` for `level=engines`.
...
#### Scenario: Improvement-only project reaches the gate and is persisted

- **WHEN** `pick-subset` selects an improvement affecting a project and no bump for that project
- **THEN** the project reaches Step 10b even though Step 10a did not invoke its bump mechanism
- **AND** its `projects[]` entry uses the level's clean no-bump fragment and records the changeset result
- **AND** the run remains eligible for persistence
