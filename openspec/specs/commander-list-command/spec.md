# commander-list-command Specification

## Purpose

Defines the `/commander:list` slash command — the read-only browser for the user-scoped Commander registry at `<HOME>/.claude/commander/projects.json`. The command renders each registered project as a YAML-ish block in insertion order, annotates legacy v1 and missing-path drift, surfaces `unsupported registry version` errors verbatim, and emits a fixed empty-registry message when no projects are registered. The command never creates, modifies, or deletes the registry file or its directory. The command is owned by the `commander` plugin.

## Requirements

### Requirement: Commander List Command File

The `commander` plugin SHALL include `commands/list.md`.

The command file SHALL have YAML frontmatter with a non-empty `description` field. The command SHALL be invocable as `/commander:list`.

#### Scenario: Command file exists

- **WHEN** examining `claude-plugins/commander/commands/`
- **THEN** `list.md` SHALL exist

#### Scenario: Frontmatter present

- **WHEN** reading `claude-plugins/commander/commands/list.md`
- **THEN** the file SHALL have YAML frontmatter with a non-empty `description` field

#### Scenario: Command invocable

- **WHEN** the user types `/commander:list`
...
- **WHEN** the user invokes `/commander:list` against an existing registry file
...
- **WHEN** `<HOME>/.claude/commander/projects.json` does not exist
- **AND** the user invokes `/commander:list`
- **THEN** the command SHALL print `No projects registered. Use /commander:add to register one.`
...
- **WHEN** the user invokes `/commander:list` with empty `ARGUMENTS`
