...
- **Primary approach (CONFIRMED, spike 2026-07-11) — orchestrator plan mode as a review/iteration UI.** The orchestrator enters plan mode, reads the teammate's `changeset.md`, and presents it via `ExitPlanMode` for the user to review/iterate. Verified to BLOCK for the human under `defaultMode: "auto"` (unlike teammate-native plan approval). **Approval does not implement in the main** — the orchestrator leaves plan mode and sends `proceed` to the still-alive teammate, which applies with its reconnaissance context intact. A reject-with-feedback is relayed as `revise: <feedback>` to the teammate, which updates `changeset.md`; the orchestrator re-presents.
- **Fallback approach — `AskUserQuestion`.** Used only where the orchestrator's plan-mode approval is unavailable or non-blocking in the active runtime (e.g. non-interactive/headless execution): `AskUserQuestion` shows the changeset digest, then the same `proceed`/`revise` relay.
...
- **Orchestrator plan-mode gate may auto-approve in `auto` mode** → RESOLVED (spike 2026-07-11): the orchestrator's own `ExitPlanMode` blocks for the human under `defaultMode: "auto"`; only the teammate path auto-resolves. `AskUserQuestion` remains the fallback for non-interactive runtimes.
...
- **Orchestrator `ExitPlanMode` gate (resolves Open Question #1):** live in the target runtime (`defaultMode: "auto"` + `skipAutoPermissionPrompt: true`). `EnterPlanMode` auto-resolved instantly, but the orchestrator's own `ExitPlanMode` BLOCKED ~39s until the human approved through the native plan UI (result: "User has approved your plan", plan marked user-reviewed) — outcome (A). On approval the main did NOT implement: it sent `proceed` to the still-alive `applier` teammate, which applied the exact approved edit (lodash `^4.17.20`→`^4.17.21` in a fixture), verified on disk by the orchestrator. Teardown needed `TaskStop` (consistent with the shutdown-request risk). Limitation: permission mode can't be switched by the agent mid-session, so `default` mode wasn't re-tested; `auto` is the more auto-approving mode, so blocking under `auto` subsumes `default`.
...
5. Validate with a dry run per level. (Gate interface already resolved by spike: plan mode primary, `AskUserQuestion` fallback for non-interactive runtimes.)
...
- ~~Does the orchestrator's own `ExitPlanMode` block for the human under `defaultMode: "auto"`, or auto-approve like the teammate path?~~ RESOLVED 2026-07-11: **(A) it blocks** under `defaultMode: "auto"` — see spike. Primary gate = orchestrator plan mode; `AskUserQuestion` demoted to fallback for non-interactive runtimes (D5).
