...
The plugin SHALL expose a `/experiments:commander-update` slash command that edits a single registered project record in place.
...
- **THEN** `/experiments:commander-update` SHALL appear in the slash-command list
...
- **WHEN** the user invokes `/experiments:commander-update investlab`
...
- **WHEN** the user invokes `/experiments:commander-update <name>` and `<name>` is not in the registry
...
- **WHEN** the user invokes `/experiments:commander-update` with no positional argument and no `--name`
