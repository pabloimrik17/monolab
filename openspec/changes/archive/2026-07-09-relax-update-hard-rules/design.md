...
- ~~The `commander-update-{minor,major,engines}` and `commander-update-deep-*` command specs match the prohibition only in _default no-op scenario assertions_. Decision: leave them; confirm at apply. Reopen only if an assertion turns out to forbid lint normatively.~~ **Resolved (reopened):** verify found these specs restate `SHALL NOT run tests/lint/build` **normatively** in their "Hard rules inherited from the orchestrator" requirement — the tripwire above. MODIFIED deltas were added for all **7** commander command specs `{minor,major,engines}` + `deep-{patch,minor,major,engines}` (dropping the clause; minor/deep-minor/deep-patch also aligned to the D6 isolation-permitting phrasing). `deep-patch` uses a bullet-list form (`SHALL NOT: - Run tests, lint, or build`) that the initial narrow verify grep missed. The `commander-update-orchestrator-skill` overview line (capability-summary prose, outside any requirement, so not delta-addressable) was reworded directly during sync.
