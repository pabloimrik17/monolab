...
**Change 5: Workflow Ordering and Validation (Added During Implementation)**

Reorder workflow to prevent dirty working directory:
```yaml
# JSR Publish (FIRST - before build artifacts exist)
- name: Publish packages to JSR
  run: |
    cd $PKG_PATH
    deno publish  # No --allow-dirty needed

# npm Publish (SECOND - after JSR is done)
- name: Build and publish packages to npm
  run: |
    pnpm nx run $PKG_DIR:build  # Generates dist/
    cd $PKG_PATH
    pnpm publish --access public  # No --no-git-checks needed
```

**Rationale:**
- npm publish requires build artifacts (`dist/`), JSR does not
- Running npm build first creates uncommitted files → dirty working directory
- JSR publish with `--allow-dirty` is a code smell that hides potential issues
- Reordering eliminates the need for both `--allow-dirty` and `--no-git-checks`
- Both tools now run their full validation suite

**Change 6: Deno Type-Checking Configuration (Added During Implementation)**

Add compilerOptions to packages using DOM APIs:
```json
...
// packages/react-clean/deno.json
{
  ...
  "exports": "./src/index.ts",
  "compilerOptions": {
    "lib": ["ES2023", "DOM"]  // Required for console, DOM APIs
  }
}
```

**Rationale:**
- Deno's type-checker is stricter than tsc
- Requires explicit `lib` configuration for DOM APIs (console, window, etc.)
- Without this, `deno publish` fails with "Cannot find name 'console'"

**Change 7: Fix ts-types Package Naming (Critical Bug Found During Implementation)**

Fix incorrect package name in Nx configuration:
```json
// packages/ts-types/project.json
{
  "name": "@m0n0lab/ts-types",  // Was: "@monolab/ts-types" (missing -lab)
  "$schema": "../../node_modules/nx/schemas/project-schema.json",
  "sourceRoot": "packages/ts-types/src",
  "projectType": "library"
}
```

**Issue Discovered:**
- `ts-types` had `@monolab/ts-types` instead of `@m0n0lab/ts-types` in `project.json`
- Nx uses `project.json` name for dependency graph, not `package.json`
- Workflow filters by `startswith("@m0n0lab/")` → ts-types was excluded
- Result: ts-types never published to JSR, breaking packages that depend on it

**Additional Fixes:**
- Updated test describe block: `describe("@m0n0lab/ts-types", ...)`
- Standardized comments: "MonoLab" → "m0n0lab" for package references
- Repository name "monolab" kept unchanged (GitHub repo name)
...
- ✅ **No Disabled Validations**: Removed `--allow-dirty` and `--no-git-checks` flags
- ✅ **Early Error Detection**: Both tools run full validation suites
- ✅ **Clean Working Directory**: Reordering prevents build artifacts from causing dirty state
- ✅ **Better CI Hygiene**: Failures indicate real problems, not configuration issues
- ✅ **Maintainability**: Future developers understand why steps are ordered this way
...
- `.github/workflows/release-please.yml`:
  ...
  - **Reorder steps**: JSR publish BEFORE npm publish
  - Change npm publish → `pnpm publish --access public` (removed `--no-git-checks`)
  - Change JSR publish → `deno publish` (removed `--allow-dirty`)
...
- `packages/ts-types/project.json`: Fix package name `@monolab` → `@m0n0lab`
...
- Add `compilerOptions.lib` to react-hooks and react-clean deno.json
- Update test file in ts-types for correct package name
