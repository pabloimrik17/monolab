...
- **Push gate integrated as an nx target (user feedback)**: instead of shell-chaining `&& pnpm run lint:fallow` after the push run-many, root package.json exposes ONLY `lint:fallow` via `nx.includedScripts` (other root scripts stay out of the graph so run-many/affected semantics are untouched) and the push step lists `lint:fallow` in the run-many targets. `cache: false` in targetDefaults: whole-repo analysis would need graph-wide cache inputs to be safe, and the run is sub-second.
