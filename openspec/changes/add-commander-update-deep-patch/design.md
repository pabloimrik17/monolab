...
### Decision 3 — `parallel-research-workflow` gains `mode` + `slugOverride`; cross-project mode changes five surfaces

In `cross-project` mode the workflow differs from `single-project` mode in exactly five surfaces, leaving the rest of the contract intact. This count matches `claude-plugins/experiments/skills/parallel-research-workflow/SKILL.md` "Mode-conditional behavior" table:
...
- **Five localized differences vs forking the skill.** A fork would duplicate the entire batching / integrity / cleanup machinery, which is the workflow's most battle-tested code (see the "hardest-learned lesson" callout in its current phase 1 description). Keeping the five surfaces co-located inside the single workflow file means a future contract change touches one set of lines, not two.
