...
The system SHALL publish packages to both npm and JSR registries automatically after release PR merge.

#### Scenario: npm publishing
- **WHEN** release PR is merged to main
- **THEN** each released package SHALL be published to npm with:
  - Scoped package name (@monolab/*)
  - Public access
  - Provenance attestation

#### Scenario: JSR publishing
- **WHEN** release PR is merged to main
- **THEN** each released package SHALL be published to JSR with:
  - @monolab scope
  - Current version from package.json

#### Scenario: Sequential publishing
- **WHEN** multiple packages are released
- **THEN** packages SHALL be published sequentially, one at a time
- **AND** publishing SHALL stop on first failure

#### Scenario: Build before publish
- **WHEN** a package needs to be published
- **THEN** the package SHALL be built using Nx before publishing
- **AND** publishing SHALL only proceed if build succeeds
