...
Knowledge recall and knowledge persistence SHALL each contribute exactly one line to the main. Recall's line SHALL be `Knowledge: <e> exact, <o> overlap, <p> prior, <r> related of <n> packages`. Persistence's line SHALL be `Knowledge: persisted <runId> → <root> (<p> packages, <h> hubs, <d> distilled, status <ok|draft>)`, or `Knowledge: not persisted (<reason>)` when persistence is skipped or fails.
